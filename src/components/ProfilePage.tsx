  ...
  return (
    <section className="bg-navy py-32" dir={ui.dir}>
      <div className="container">
        <Link href={localized("/team", lang)} className="text-sm uppercase tracking-[.15em] text-gold hover:text-ivory">{lang === "he" ? "חזרה לצוות" : "Back to team"}</Link>
        <div className="mt-10 grid gap-12 lg:grid-cols-[360px_1fr]">
          <div className="lawyer-placeholder aspect-[3/4] overflow-hidden bg-rule">
          ...
          <article>
            ...
            <div className="space-y-5">
              {bio.map((p) => <p key={p} className="body-text text-base">{p}</p>)}
            </div>
            <div className="mt-12 grid gap-10 md:grid-cols-2">
              <section>
                <h2 className="mb-4 font-display text-3xl font-normal text-ivory">{lang === "he" ? "השכלה" : "Education"}</h2>
                <ul className="m-0 space-y-3 p-0 text-sm leading-7 text-stone">
                  {education.map((x) => <li key={x}>{x}</li>)}
                </ul>
              </section>
              <section>
                <h2 className="mb-4 font-display text-3xl font-normal text-ivory">{lang === "he" ? "הסמכה" : "Admissions"}</h2>
                <ul className="m-0 space-y-3 p-0 text-sm leading-7 text-stone">
                  {admission.map((x) => <li key={x}>{x}</li>)}
                </ul>
              </section>
            </div>
            <p className="mt-10 text-sm text-stone"><a href={`mailto:${member.email}`} className="text-gold hover:text-ivory">{member.email}</a></p>
          </article>
        </div>
      </div>
    </section>
