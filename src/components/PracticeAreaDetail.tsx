    ...
    return (
      <section className="bg-navy py-28" dir={ui.dir}>
        <div className="narrow">
          ...
          <h1 className="display-title">{lang === "he" ? "התחום לא נמצא" : "Practice area not found"}</h1>
          <p className="body-text mt-8">
            {lang === "he" ? "העמוד המבוקש אינו קיים." : "The requested page does not exist."}
          </p>
          <Link className="mt-10 inline-block text-gold no-underline hover:text-ivory" href={localized("/practice-areas", lang)}>
    ...
    <section className="bg-navy py-28" dir={ui.dir}>
      <div className="narrow">
        ...
        <article className="mt-14 border-y border-rule py-12">
          <p className="body-text m-0 text-lg leading-[2]">{area.desc}</p>
        </article>

        <div className="mt-12 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <Link className="text-sm uppercase tracking-[.14em] text-gold no-underline hover:text-ivory" href={localized("/practice-areas", lang)}>
          ...
          <div className="flex flex-wrap gap-3">
            {content.items.map((item) => (
              <Link
                ...
                className={`border px-4 py-2 text-sm no-underline transition-colors ${item.slug === slug ? "border-gold text-gold" : "border-rule text-stone hover:border-gold hover:text-ivory"}`}
              >
                {item.name}
              </Link>
            ))}
