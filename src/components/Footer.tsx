...
export default function Footer({ lang }: { lang: Lang }) {
  ...
  const address = lang === "he" ? SITE.addressHe : SITE.addressEn;
      ...
      <div className="container py-16">
        <div className="grid gap-12 md:grid-cols-3">
          <div>
            ...
            <p className="mt-2 font-display text-lg text-stone">
              {ui.descriptor}
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="m-0 list-none space-y-3 p-0 text-sm leading-7 text-stone">
              <li>
                <a href={phoneHref(SITE.phone)} className="hover:text-ivory">
                  {SITE.phone}
                </a>
              </li>
              <li>
                <a href={mailHref(SITE.email)} className="hover:text-ivory">
                  {SITE.email}
                </a>
              </li>
              <li>
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-rule pt-8 text-xs text-muted md:flex-row md:items-center md:justify-between">
          <p className="m-0">
            © {year} {ui.firm}. {ui.rights}.
          </p>
          <Link
            href={localized("/accessibility", lang)}
            className="tracking-[.12em] text-stone hover:text-ivory"
          >
            {ui.accessibility}
          </Link>
        </div>
